...
def find_arrow_file(root_dir: Path) -> Path:
    ...
    if preferred:
        return preferred[0]
    if len(files) > 1:
        print(f"注意：找到多个Arrow文件，按排序选择：{files[0]}")
    return files[0]
...
class SieveFilterBank:
    ...
    def best_result(self) -> Dict[str, object]:
        ...
        return {
            ...
            "score": float(self.scores[best]),
            "normalized_score": float(self.scores[best] / self.coverage.shape[1]),
            "budget": int(self.budget),
            "selected_indices": list(self.selected_indices[best]),
            "accepted_gains": list(self.accepted_gains[best]),
...
def write_metadata(
    ...
    summary = {
        "algorithm": "Sieve-Streaming with anchor-based DTW facility location",
        "arrow_path": str(arrow_path),
        "dataset_output": str(output_root / "dataset"),
        "seed": args.seed,
        "selection_ratio_upper_bound": args.selection_ratio,
        "epsilon": args.epsilon,
        "anchor_size": args.anchor_size,
        "downsample_len": args.downsample_len,
...
def main() -> None:
    ...
    print(
        f"DTW: C/OpenMP={not args.no_dtw_parallel}, "
    ...
    # 对应算法B第15步：每个标签选取得分最高的过滤器。
    results = {label: bank.best_result() for label, bank in banks.items()}
    print("\n最优过滤器")
    for label in sorted(results):
        result = results[label]
        print(
